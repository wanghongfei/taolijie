...
CREATE TABLE IF NOT EXISTS `taoljie`.`member` (
  ...
  `college_id` INT NULL,
  `verified` INT NULL COMMENT '是否通过认证',
  PRIMARY KEY (`id`),
...
-- -----------------------------------------------------
-- Table `taoljie`.`post_category`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `taoljie`.`post_category` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `name` VARCHAR(45) NULL,
  `memo` TEXT NULL,
  PRIMARY KEY (`id`))
...
CREATE TABLE IF NOT EXISTS `taoljie`.`post` (
  ...
  `passtime` DATE NULL,
  `time` DATE NULL,
  `location` VARCHAR(45) NULL COMMENT 'e.g., 山东省:淄博市:张店区',
  `wage` INT NULL,
  ...
  `member_id` INT NULL,
  `post_category_id` INT NULL COMMENT 'FK: 兼职分类表',
  PRIMARY KEY (`id`),
  ...
  CONSTRAINT `fk_post_post_category1`
    ...
    REFERENCES `taoljie`.`post_category` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB
COMMENT = '二手信息';
