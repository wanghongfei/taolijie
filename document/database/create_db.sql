...
CREATE TABLE IF NOT EXISTS `taolijie`.`member` (
  ...
  `verified` VARCHAR(3) NULL DEFAULT '00' COMMENT '00: 未审核 01: 已审核',
...
CREATE TABLE IF NOT EXISTS `taolijie`.`quest_assign` (
  `id` INT NOT NULL AUTO_INCREMENT COMMENT '',
  `member_id` INT NULL COMMENT '领取者用户id',
  `username` VARCHAR(45) NULL COMMENT '领取者用户名',
  `quest_id` INT NULL COMMENT '任务id',
  `quest_title` VARCHAR(45) NULL COMMENT '任取的任务的标题',
  `assign_time` DATETIME NULL COMMENT '领取时间',
  `status` VARCHAR(3) NULL DEFAULT '00' COMMENT '00: 已领取. 01: 已完成 02: 已结束',
  PRIMARY KEY (`id`)  COMMENT '',
...
CREATE TABLE IF NOT EXISTS `taolijie`.`cash_acc` (
  ...
  `deal_password` VARCHAR(20) NULL COMMENT '交易密码',
...
CREATE TABLE IF NOT EXISTS `taolijie`.`acc_flow` (
  `id` INT NOT NULL AUTO_INCREMENT COMMENT '',
  `created_time` DATETIME NULL COMMENT '',
  `acc_id` INT NULL COMMENT '',
  `action_type` VARCHAR(3) NULL COMMENT '00: 提现 01:充值 02:冻结',
...
CREATE TABLE IF NOT EXISTS `taolijie`.`finish_request` (
  `id` INT NOT NULL AUTO_INCREMENT COMMENT '',
  `quest_id` INT NULL COMMENT '任务id',
  `member_id` INT NULL COMMENT '申请者用户id',
  ...
  `status` VARCHAR(3) NULL DEFAULT '01' COMMENT '00: 已通过 01: 等待审核 02: 审核未通过',
...
CREATE TABLE IF NOT EXISTS `taolijie`.`pay_order` (
  `id` INT NOT NULL AUTO_INCREMENT COMMENT '',
  `cash_acc_id` INT NULL COMMENT '订单对应的现金账户',
  `member_id` INT NULL COMMENT '订单生成者id',
  `title` VARCHAR(45) NULL COMMENT '订单标题',
  `created_time` DATETIME NULL COMMENT '订单创建时间',
  `update_time` DATETIME NULL COMMENT '状态更新时间 ',
  `amount` DECIMAL(9,2) NULL COMMENT '订单金额',
  `type` VARCHAR(3) NULL COMMENT '订单类型. 00: 充值订单',
  `status` VARCHAR(3) NULL COMMENT '订单状态. 00:等待审核 01: 交易成功 02: 审核未通过',
  `alipay_trade_num` VARCHAR(35) NULL COMMENT '支付宝交易号',
