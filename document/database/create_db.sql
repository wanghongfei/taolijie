...
CREATE TABLE IF NOT EXISTS `taolijie`.`quest_assign` (
  ...
  `status` VARCHAR(3) NULL COMMENT '00: 正在进行. 01: 已完成 02: 已派现 03: 已经结束',
  PRIMARY KEY (`id`)  COMMENT '',
...
CREATE TABLE IF NOT EXISTS `taolijie`.`withdraw_apply` (
  ...
  `status` VARCHAR(3) NULL COMMENT '00: 等待审核 01: 等待派现 02: 已完成 03: 提现失败',
...
CREATE TABLE IF NOT EXISTS `taolijie`.`cash_acc` (
  ...
  `status` VARCHAR(3) NULL COMMENT '00: 正常 01: 冻结',
